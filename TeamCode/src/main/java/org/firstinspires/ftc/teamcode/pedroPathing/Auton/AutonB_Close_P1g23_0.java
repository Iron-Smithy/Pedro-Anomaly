...
public class AutonB_Close_P1g23_0 extends OpMode {
    ...
    private final Pose gateHitPoseRed1 = new Pose(126.5, 78.5, Math.toRadians(0)); // Y was 73.5
    private final Pose gateHitBackUpCPPoseRed1 = new Pose(117.5, 78.5, Math.toRadians(0)); // Y was 68.5
...
//    private final Pose goalPose = pose(new Pose(132, 136, 0));
    ...
    private PathChain row1toGateHit;
    ...
    private void buildPaths() { // this function adapts poses to the proper alliance side, then generates the paths the robot should follow during autonomous
        ...
        Pose gateHitPose1 = pose(gateHitPoseRed1);
        Pose gateHitBackUpCPPose1 = pose(gateHitBackUpCPPoseRed1);

        goalPose = pose(new Pose(142, 142, 0)); //MConstants.goalPoseRed
        ...
        row1toGateHit = follower.pathBuilder()
                .addPath(new BezierCurve(r1Collect, gateHitBackUpCPPose1, gateHitPose1))
                .setConstantHeadingInterpolation(r1Collect.getHeading())
                .build();

        row1Return = follower.pathBuilder()
                .addPath(new BezierCurve(gateHitPose1, scoreR1CP, scorePose))
                .setLinearHeadingInterpolation(gateHitPose1.getHeading(), scorePose.getHeading())
                .build();
...
//                aimAtTarget(currentPose); // aim turret at the goal
                ...
                if (!follower.isBusy() && pathTimer.getElapsedTime() > 2000) { //RobotUtils.isStable(follower)     // once the robot has reached the target position
                    blocker.in();
                    fireTask = new AutoFireTask(outtake, indexer, ejector, intake, ballSensors, scoreShooterTPS, 4); // set up a new shooting program
...
//                aimAtTarget(currentPose); // keep updating aim in case the robot still moving
                ...
                if (fireTask.isActive()) {
                    ...
                    follower.followPath(row1Pickup, 1, true);
            ...
            case PICKUP_R1:
                if (!follower.isBusy()) { // once finished collecting objects
                    ...
                    follower.followPath(row1toGateHit, 1, true);
                    transitionTo(AutoState.HIT_GATE1);
                }
                break;
            case HIT_GATE1:
...
//                    intake.runInAt(0.3); // was .45. slow intake to put less pressure on intake, motors, blockers etc. but keep spinning to catch balls not fully in
                    ...
                    transitionTo(AutoState.HIT_GATE_HOLD1);
                } else if (pathTimer.getElapsedTime() > 3000) {
                    indexer.stop(); // to not drain battery
//                    intake.runInAt(0.3);
                    follower.followPath(row1Return, 1, true); // was 0.7
                    transitionTo(AutoState.GO_SCORE_R1);
                }
                break;
            case HIT_GATE_HOLD1:
                if (pathTimer.getElapsedTime() > 500) {
                    follower.followPath(row1Return, 1, false); // was 0.7
                    transitionTo(AutoState.GO_SCORE_R1);
                }
                break;
            case GO_SCORE_R1:
...
//                aimAtTarget(currentPose);
            ...
            case PICKUP_R2:
                if (!follower.isBusy()) {
                    intake.runInAt(0.50);
//                    intake.stop();
                    follower.followPath(row2Return, 1, true);
...
//                aimAtTarget(currentPose);
                ...
                if (fireTask.isActive()) {
                    ...
                    intake.runIn();
                    follower.followPath(row3Pickup, 1, true); // row3Pickup
            ...
            case PICKUP_R3:
                if (!follower.isBusy()) {
                    intake.runInAt(0.5);
                    follower.followPath(row3Return, 1, true); // row3Return
